...
def chat():
    ...
    try:
        ...
        prompt_chat = f"TODAY IS {today_str}. IDENTITY: CRPM Engine. CONTEXT: {context}. QUERY: {user_query}. Respond in English."
    ...
    try:
        ...
        return jsonify({
            ...
            "machines": [
                ...
                {"name": "Machine 2: Short Put Based", "action": "SELL PUT", "strike": s_put_30, "expiry": exp_30, "prem": f2(p_put_30), "max_profit": f"${f2(p_put_30*100)}", "max_risk": f"${f2((s_put_30-p_put_30)*100)}", "comment": "Income.", "desc": "Volatility harvest."},
                {"name": "Machine 3: Married Put Based", "action": "BUY PUT (+100)", "strike": s_put_180, "expiry": exp_180, "prem": f2(p_put_180), "max_profit": "UNLIMITED", "max_risk": f"${f2((p_put_180+(price-s_put_180))*100)}", "comment": "Hedging.", "desc": "Protection."},
                ...
                {"name": "Machine 5: Combined", "action": "PUT & CALL", "strike": f"{s_put_30}/{s_call}", "expiry": exp_30, "prem": f2(p_call + p_put_30), "max_profit": "Yield+", "max_risk": "Basis-", "comment": "Cost reduction.", "desc": "Profit."}
            ]
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500

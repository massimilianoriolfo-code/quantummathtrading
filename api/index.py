...
def chat():
    ...
    try:
        ...
        prompt_chat = f"CONTEXT: {context}. QUERY: {user_query}. Respond in English."
...
def index():
    ...
    if not ticker_sym: return jsonify({"error": "Ticker missing"}), 400
    today_dt = get_now()
    try:
        ...
        return jsonify({
            "ticker": ticker_sym, "company": stock.info.get('longName', ticker_sym), 
            "price": f2(price), "inv_cap": f2(inv_cap), "volatility": 27.0, 
            "high": s_call, "low": s_put30, "date": today_dt.strftime('%B %d, %Y'),
            "machines": [
                ...
                {"name": "Machine 5", "action": "PUT & CALL", "strike": f"{s_put30}/{s_call}", "expiry": exp_30, "prem": f2(p_call + p_put30), "max_profit": "Yield+", "max_risk": "Basis-", "comment": "Cost reduction.", "desc": "Profit."}
            ]
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
